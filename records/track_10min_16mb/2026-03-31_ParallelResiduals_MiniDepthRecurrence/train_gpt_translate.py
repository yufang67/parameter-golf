...
class Hyperparameters:
	...
	tied_embed_lr=float(os.environ.get('TIED_EMBED_LR',.035))
	tied_embed_init_std=float(os.environ.get('TIED_EMBED_INIT_STD',.005))
	matrix_lr=float(os.environ.get('MATRIX_LR',.03))
	matrix_lr_early=float(os.environ.get('MATRIX_LR_EARLY',.025))
	matrix_lr_late=float(os.environ.get('MATRIX_LR_LATE',.03))
	bank_split=int(os.environ.get('BANK_SPLIT',5))
	scalar_lr=float(os.environ.get('SCALAR_LR',.025))
	...
	muon_wd=float(os.environ.get('MUON_WD',.04))
	adam_wd=float(os.environ.get('ADAM_WD',.04))
	...
	ttt_lr=float(os.environ.get('TTT_LR',.002))
	...
	negative_slope=float(os.environ.get('NEGATIVE_SLOPE',.5))
	...
	late_qat_threshold=float(os.environ.get('LATE_QAT_THRESHOLD',.15))
...
class DistributedTokenLoader:
	...
	def _sample_global_windows(self):
		...
		progress=min(self._batches_built/18e2,1.)
...
def apply_canon_residual(x,w):
	...
	y=x*w[0][None,None,:]
	y=y+F.pad(x[:,:-1],(0,0,1,0))*w[1][None,None,:]
	y=y+F.pad(x[:,:-2],(0,0,2,0))*w[2][None,None,:]
	y=y+F.pad(x[:,:-3],(0,0,3,0))*w[3][None,None,:]
	return x+y
class CastedLinear(nn.Linear):
	...
	def forward(self,x):
		...
		if CastedLinear._qat_enabled and self.training and w.ndim==2:
			...
			s=(row_max/31.).clamp_min(1./31.)
			scaled=w32/s[:,None]
			alpha=CastedLinear._qat_alpha
			frac=scaled-scaled.floor()
			soft_rounded=scaled.floor()+torch.sigmoid(alpha*(frac-.5))
			w_q=(torch.clamp(soft_rounded,-31,31)*s[:,None]).to(x.dtype)
			w=w_q
...
class CausalSelfAttention(nn.Module):
	...
	def forward(self,x,q_w,k_w,v_w,out_w,v_embed=None):
		if getattr(self,'_save_gptq',False):self._gptq_qkv_in=x.detach()
		...
		if getattr(self,'_save_gptq',False):self._gptq_o_in=y.detach()
...
class ValueEmbedding(nn.Module):
	def __init__(self,vocab_size,ve_dim,model_dim):
		...
		nn.init.normal_(self.embed.weight,std=.01)
	...
	def forward(self,token_ids):
		...
		return h*self.scale.to(dtype=h.dtype)
class MLP(nn.Module):
	def __init__(self,dim,mlp_mult,neg_slope=.5):
		super().__init__()
		self.neg_slope=neg_slope
	def forward(self,x,up_w,down_w):
		if getattr(self,'_save_gptq',False):self._gptq_up_in=x.detach()
		x=F.leaky_relu(F.linear(x,up_w.to(x.dtype)),negative_slope=self.neg_slope)
		x2=x.square()
		if getattr(self,'_save_gptq',False):self._gptq_down_in=x2.detach()
...
class Block(nn.Module):
	def __init__(self,dim,num_heads,num_kv_heads,mlp_mult,rope_base,qk_gain_init,layer_idx=0,ln_scale=False,neg_slope=.5,disable_attn=False):
		super().__init__()
	...
	def forward(self,x,x0,q_w,k_w,v_w,out_w,up_w,down_w,v_embed=None,canon_a_w=None,canon_c_w=None):
		mix=self.resid_mix.to(dtype=x.dtype)
		x_in=mix[0][None,None,:]*x+mix[1][None,None,:]*x0
		x_out=x_in
		if not self.disable_attn:
			attn_in=self.attn_norm(x_in)*self.ln_scale_factor
			if canon_a_w is not None:attn_in=apply_canon_residual(attn_in,canon_a_w)
		...
		if canon_c_w is not None:mlp_in=apply_canon_residual(mlp_in,canon_c_w)
		mlp_out=self.mlp_scale.to(dtype=x_out.dtype)[None,None,:]*self.mlp(mlp_in,up_w,down_w)
		return x_out+mlp_out
class GPT(nn.Module):
	def __init__(self,vocab_size,num_layers,model_dim,num_heads,num_kv_heads,mlp_mult,tie_embeddings,tied_embed_init_std,logit_softcap,rope_base,qk_gain_init,bigram_vocab_size=0,bigram_dim=128,xsa_last_n=0,rope_dims=0,ln_scale=False,ve_enabled=False,ve_dim=128,ve_layers='9,10',canon_ac_layers=None,parallel_residual=False,parallel_start_layer=7,parallel_start_layer_is_physical=True,neg_slope=.5,disable_layer0_attn=False,recur_layers=None,recurrence_active=False,repeat_untie_mlp='none',repeat_untie_mlp_layers=None):
		super().__init__()
		...
		self.canon_a=nn.Parameter(torch.zeros(num_layers,4,model_dim,dtype=torch.float32))if self.canon_ac_layers else None
		self.canon_c=nn.Parameter(torch.zeros(num_layers,4,model_dim,dtype=torch.float32))if self.canon_ac_layers else None
		self.parallel_post_lambdas=nn.Parameter(torch.ones(num_layers,2,2,dtype=torch.float32))if self.parallel_residual else None
		self.parallel_resid_lambdas=nn.Parameter(torch.full((num_layers,2),1.1**.5,dtype=torch.float32))if self.parallel_residual else None
		...
		if xsa_last_n>0:
			for i in range(max(0,self.virtual_num_layers-xsa_last_n),self.virtual_num_layers):self.blocks[i].attn.use_xsa=True
	...
	def _init_weights(self):
		...
		n=self.num_layers
		proj_scale=1./math.sqrt(2*n)
		for i in range(n):
			nn.init.orthogonal_(self.qo_bank.data[i],gain=1.)
			nn.init.zeros_(self.qo_bank.data[n+i])
			nn.init.orthogonal_(self.kv_bank.data[i],gain=1.)
			nn.init.orthogonal_(self.kv_bank.data[n+i],gain=1.)
			nn.init.orthogonal_(self.mlp_up_bank.data[i],gain=1.)
			nn.init.zeros_(self.mlp_down_bank.data[i])
			self.qo_bank.data[n+i].mul_(proj_scale)
			self.mlp_down_bank.data[i].mul_(proj_scale)
		for repeat_mlp in self.repeat_mlp:
			if repeat_mlp.fc is not None:nn.init.zeros_(repeat_mlp.fc.weight)
			if repeat_mlp.proj is not None:nn.init.zeros_(repeat_mlp.proj.weight)
	...
	def set_recurrence_active(self,active):
		was_active=getattr(self,'_recurrence_active',False)
	...
	def _sync_repeat_mlp_from_base(self):
		with torch.no_grad():
			for(repeat_idx,physical_idx)in enumerate(self.recur_layers):
				...
				if repeat_mlp.fc is not None:repeat_mlp.fc.weight.copy_(self.mlp_up_bank[physical_idx])
				if repeat_mlp.proj is not None:repeat_mlp.proj.weight.copy_(self.mlp_down_bank[physical_idx])
	...
	def _parallel_active_for_layer(self,virtual_idx):
		if self.parallel_post_lambdas is None:return False
	...
	def _apply_skip_parallel(self,lane0,lane1,skip,i):
		...
		g=torch.sigmoid(self.skip_gates[i].to(dtype=lane0.dtype))[None,None,:]
		w=self.skip_weights[i].to(dtype=lane0.dtype)[None,None,:]
	...
	def _parallel_block(self,virtual_idx,lane0,lane1,x0,q_w,k_w,v_w,out_w,up_w,down_w,v_embed=None,canon_a_w=None,canon_c_w=None):
		...
		if canon_c_w is not None:mlp_in=apply_canon_residual(mlp_in,canon_c_w)
		mlp_out=block.mlp_scale.to(dtype=lane1.dtype)[None,None,:]*block.mlp(mlp_in,up_w,down_w)
	...
	def _get_block_weights(self,virtual_idx):
		...
		canon_a_w=self.canon_a[physical_idx]if self.canon_a is not None and physical_idx in self._canon_ac_layer_set else None
		canon_c_w=self.canon_c[physical_idx]if self.canon_c is not None and physical_idx in self._canon_ac_layer_set else None
		if self._is_repeated_virtual_index(virtual_idx):
			...
			if self.repeat_mlp:
				...
				if repeat_mlp.fc is not None:up_w=repeat_mlp.fc.weight
				if repeat_mlp.proj is not None:down_w=repeat_mlp.proj.weight
	...
	def _backbone(self,input_ids):
		...
		for i in range(self.num_encoder_layers):
			...
			if self._parallel_active_for_layer(i):
				if lane0 is None:lane0=lane1=x
		...
		return self.final_norm(self._final_parallel_hidden(lane0,lane1) if lane1 is not None else x)
...
def eval_val_sliding(args,base_model,rank,world_size,device,val_tokens,base_bytes_lut,has_leading_space_lut,is_boundary_token_lut,stride,batch_seqs=32,eval_seq_len=None):
	...
	for bi in range(0,len(my_windows),batch_seqs):
		...
		if use_slot:
			with torch.no_grad(),torch.autocast(device_type='cuda',dtype=torch.bfloat16):H=compiled_hidden(x_batch)
			H=H.detach().float()
			...
			with torch.inference_mode(),torch.autocast(device_type='cuda',dtype=torch.bfloat16):logits=compiled_logits(x_batch)
		with torch.no_grad():
...
def eval_val_sliding_ttt(args,base_model,rank,world_size,device,val_tokens,base_bytes_lut,has_leading_space_lut,is_boundary_token_lut,stride,batch_seqs=32,log0=print):
	...
	for ci in range(num_chunks):
		...
		if not is_last_chunk and args.ttt_epochs>0:
			...
			if chunk_seqs>0:
				cos_lr=args.ttt_lr*.5*(1.+math.cos(math.pi*ci/max(num_chunks-1,1)))
		...
		if rank==0 and(ci%10==0 or ci==num_chunks-1):
			...
			rbpb=rl/math.log(2.)*(token_count.item()/max(byte_count.item(),1))if token_count.item()>0 else .0
	...
	base_model.eval()
	log0(f"ttt_sliding:done val_loss={val_loss:.6f}{ val_bpb=:.6f} elapsed={time.perf_counter()-t0:.1f}s")
...
def _classify_param(name):
	...
	if '.mlp.' in name or name.startswith('repeat_mlp.'):return'mlp'
...
def log_parallel_residual_converged(log0,model):
	if getattr(model,'parallel_post_lambdas',None)is None:return
	...
	mode='physical'if getattr(model,'parallel_start_layer_is_physical',False)else'virtual'
...
def mixed_quantize_int6(state_dict,int6_cats,clip_range=31,hessians=None,clip_ranges=None):
	...
	for(name,tensor)in state_dict.items():
		...
		if cat in int6_cats and t.ndim>=1:
			...
			result[name+'.q']=q
			result[name+'.scale']=s
			meta[name]={'type':'int6'if cr>=31 else 'int5'}
		else:
			q,s=quantize_float_tensor(t)
			result[name+'.q']=q
			result[name+'.scale']=s
			meta[name]={'type':'int8'}
	if hessians:print(f"gptq_quantize: {gptq_count} GPTQ layers, {naive_count} naive layers",flush=True)
	return result,meta
...
def gptq_quantize_weight(W,H,clip_range=31,block_size=128,percdamp=.01):
	...
	for pct in[.999,.9995,.9999,.99999,1.]:
		if pct<1.:row_clip=torch.quantile(W_orig.abs(),pct,dim=1)
...
def _init_hessians(base_model,dim,mlp_dim,device):
	...
	for(repeat_idx,repeat_mlp)in enumerate(base_model.repeat_mlp):
		if repeat_mlp.fc is not None:h[f"repeat_mlp.{repeat_idx}.fc.weight"]=torch.zeros(dim,dim,dtype=torch.float32,device=device)
		if repeat_mlp.proj is not None:h[f"repeat_mlp.{repeat_idx}.proj.weight"]=torch.zeros(mlp_dim,mlp_dim,dtype=torch.float32,device=device)
	return h
def _accum_hessians(hessians,base_model,dim,mlp_dim):
	for(i,block)in enumerate(base_model.blocks):
		physical_idx=base_model.v2p[i]
		if not getattr(block,'disable_attn',False) and hasattr(block.attn,'_gptq_qkv_in') and hasattr(block.attn,'_gptq_o_in'):
		...
		if hasattr(block.mlp,'_gptq_up_in') and hasattr(block.mlp,'_gptq_down_in'):
			...
			if base_model._is_repeated_virtual_index(i)and base_model.repeat_mlp:
				...
				if repeat_mlp.fc is not None:up_name=f"repeat_mlp.{repeat_idx}.fc.weight"
				if repeat_mlp.proj is not None:down_name=f"repeat_mlp.{repeat_idx}.proj.weight"
...
def _finalize_hessians(hessians,num_batches):
	for name in hessians:
		...
		damp=.01*torch.diag(hessians[name]).mean().clamp_min(1e-06)
...
def main():
	...
	if '__wrapper_size__' in dir()or '__wrapper_size__' in globals():code='x'*globals().get('__wrapper_size__',len(code))
	...
	if args.post_gptq_eval_only:
		...
		return
	base_model=GPT(vocab_size=args.vocab_size,num_layers=args.num_layers,model_dim=args.model_dim,num_heads=args.num_heads,num_kv_heads=args.num_kv_heads,mlp_mult=args.mlp_mult,tie_embeddings=args.tie_embeddings,tied_embed_init_std=args.tied_embed_init_std,logit_softcap=args.logit_softcap,rope_base=args.rope_base,qk_gain_init=args.qk_gain_init,bigram_vocab_size=args.bigram_vocab_size,bigram_dim=args.bigram_dim,xsa_last_n=args.xsa_last_n,rope_dims=args.rope_dims,ln_scale=args.ln_scale,ve_enabled=args.ve_enabled,ve_dim=args.ve_dim,ve_layers=args.ve_layers,canon_ac_layers=canon_ac_layers,parallel_residual=args.parallel_residual,parallel_start_layer=args.parallel_start_layer,parallel_start_layer_is_physical=args.parallel_start_layer_is_physical,neg_slope=args.negative_slope,disable_layer0_attn=args.disable_layer0_attn,recur_layers=recur_layers,recurrence_active=False,repeat_untie_mlp=args.repeat_untie_mlp,repeat_untie_mlp_layers=repeat_untie_mlp_layers).to(device).bfloat16()
	...
	if base_model.canon_a is not None:
	...
	log0(f"canon_ac:layers={canon_ac_layers} params={0 if base_model.canon_a is None else base_model.canon_a.numel()+base_model.canon_c.numel()} physical_only=1")
	log0(f"parallel_residual:active={int(base_model.parallel_post_lambdas is not None)} start_layer={base_model.parallel_start_layer} start_mode={'physical'if base_model.parallel_start_layer_is_physical else 'virtual'} params={0 if base_model.parallel_post_lambdas is None else base_model.parallel_post_lambdas.numel()+base_model.parallel_resid_lambdas.numel()} final_lane=mlp")
	...
	def lr_mul(step,elapsed_ms):
		...
		if max_wallclock_ms is None:
			...
			return max((args.iterations-step)/max(args.warmdown_iters,1),.0)if warmdown_start<=step<args.iterations else 1.
		step_ms=elapsed_ms/max(step,1)
		warmdown_ms=args.warmdown_iters*step_ms
		remaining_ms=max(max_wallclock_ms-elapsed_ms,.0)
		return remaining_ms/max(warmdown_ms,1e-09)if remaining_ms<=warmdown_ms else 1.
	...
	while True:
		...
		if args.late_qat_threshold>0 and scale<args.late_qat_threshold and step>=2000:
			...
			qat_progress=min((step-CastedLinear._qat_start_step)/max(500,1),1.)
			CastedLinear._qat_alpha=1.+15.*qat_progress
	...
	torch.cuda.synchronize()
	log0(f"DIAGNOSTIC post_ema val_loss:{diag_val_loss:.4f} val_bpb:{diag_val_bpb:.4f} eval_time:{1e3*(time.perf_counter()-t_diag):.0f}ms")
	...
	if args.use_gptq:
		...
		log0(f"gptq:calibration recurrence_active={int(base_model._recurrence_active)} repeat_mlp={len(base_model.repeat_mlp)} parallel_residual={int(base_model.parallel_post_lambdas is not None)} ar_selfgen={int(args.gptq_ar_selfgen)}")
	...
	clip_ranges=None
	if args.mixed_quant and gptq_hessians is not None:
	...
	q_val_loss,q_val_bpb=eval_val(args,eval_model,rank,world_size,device,grad_accum_steps,val_tokens,base_bytes_lut,has_leading_space_lut,is_boundary_token_lut,eval_seq_len=effective_eval_seq_len)
	torch.cuda.synchronize()
	log0(f"final_int6_roundtrip val_loss:{q_val_loss:.4f} val_bpb:{q_val_bpb:.4f} eval_time:{1e3*(time.perf_counter()-t_qeval):.0f}ms")
	log0(f"final_int6_roundtrip_exact val_loss:{q_val_loss:.8f} val_bpb:{q_val_bpb:.8f}")
	sw_seq_len=effective_eval_seq_len
	if args.eval_stride>0 and args.eval_stride<sw_seq_len:
		torch.cuda.synchronize()
		...
		sw_val_loss,sw_val_bpb=eval_val_sliding(args,eval_model,rank,world_size,device,val_tokens,base_bytes_lut,has_leading_space_lut,is_boundary_token_lut,stride=args.eval_stride,eval_seq_len=sw_seq_len)
		torch.cuda.synchronize()
		log0(f"final_int6_sliding_window val_loss:{sw_val_loss:.4f} val_bpb:{sw_val_bpb:.4f} stride:{args.eval_stride} eval_time:{1e3*(time.perf_counter()-t_slide):.0f}ms")
		log0(f"final_int6_sliding_window_exact val_loss:{sw_val_loss:.8f} val_bpb:{sw_val_bpb:.8f}")
		log0(f"final_int8_zlib_roundtrip_exact val_loss:{sw_val_loss:.8f} val_bpb:{sw_val_bpb:.8f}")
	if args.eval_stride!=64 and 64<sw_seq_len:
		torch.cuda.synchronize()
		...
		sw64_val_loss,sw64_val_bpb=eval_val_sliding(args,eval_model,rank,world_size,device,val_tokens,base_bytes_lut,has_leading_space_lut,is_boundary_token_lut,stride=64,eval_seq_len=sw_seq_len)
		torch.cuda.synchronize()
		log0(f"final_int6_sliding_window_s64 val_loss:{sw64_val_loss:.4f} val_bpb:{sw64_val_bpb:.4f} stride:64 eval_time:{1e3*(time.perf_counter()-t_slide64):.0f}ms")
		log0(f"final_int6_sliding_window_s64_exact val_loss:{sw64_val_loss:.8f} val_bpb:{sw64_val_bpb:.8f}")
		log0(f"final_int8_zlib_roundtrip_exact val_loss:{sw64_val_loss:.8f} val_bpb:{sw64_val_bpb:.8f}")
	if args.ttt_enabled:
		torch.cuda.synchronize()
		...
		torch.cuda.synchronize()
		log0(f"legal_ttt val_loss:{ttt_loss:.4f} val_bpb:{ttt_bpb:.4f} eval_time:{1e3*(time.perf_counter()-t_ttt):.0f}ms")
