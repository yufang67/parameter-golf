...
**Root cause:** **RoPE base-seqlen mismatch between training and TTT eval.**

Under `VARLEN_ATTENTION=1`, the `DocumentPackingLoader` packs many documents into long rows (≈ `train_batch_tokens / (world * grad_accum)` ≫ `train_seq_len`). Because that exceeds `train_seq_len`, `Rotary.forward` recomputes cos/sin under the **NTK-aware rescaled base** (`new_base = base * (scale ** (rd / (rd - 2)))`). The trained weights therefore expect that rescaled RoPE.

The TTT eval path (`eval_val_ttt`) processes 2048-token rows that are *shorter* than the training packed rows, so `Rotary.forward` returned the **vanilla cached** cos/sin instead. Result: the LoRA-TTT forward saw RoPE encodings that did not match what the weights had been trained against → the running BPB drifted up to ≈1.20 instead of improving on the ≈1.074 sliding baseline.

This also explains why every varlen eval path (sliding-window, fullparam-TTT, LoRA-TTT) had a slightly worse quant→sw delta than its non-varlen counterpart — they all touch the same `Rotary.forward` code with the same base-seqlen confusion at eval-time. LoRA-TTT just amplified it because the per-chunk gradient steps actively chase the wrong target.

**Fix** (landed in `train_gpt_improved_04_16.py`):
- New env var `ROPE_FORCE_BASE_SEQLEN`: when >0, `Rotary.forward` recomputes cos/sin as if `seq_len == ROPE_FORCE_BASE_SEQLEN` (taking the NTK rescale path when that exceeds `train_seq_len`).
- New hyperparam `TTT_ROPE_BASE_SEQLEN` (default `0` = auto): on the varlen LoRA-TTT branch in `train_and_eval`, before `eval_val_ttt`, the script auto-sets `ROPE_FORCE_BASE_SEQLEN = max(train_seq_len + 1, train_batch_tokens // (world * grad_accum))` and restores the previous value after eval. Logged as `ttt_lora:rope_base_fix ROPE_FORCE_BASE_SEQLEN=...`.
- Override `TTT_ROPE_BASE_SEQLEN=<int>` only if the auto value doesn't match what training actually saw (e.g. exotic batch shapes).

**Result on `pg12_varlen_clip14`** (sliding baseline 1.07425):

| Run | Vars (overrides on default) | quantized_ttt_lora val_bpb |
|---|---|---|
| `pg12_ttt_r48_phased2`        | `TTT_LORA_RANK=48 TTT_PHASES=2` | **1.07183** ✅ best |
...
| (target from original table)  | `Varlen + TTT target` | ≤ 1.071 |
| (record from non-varlen track) | `improved_GA_..._TTT` | 1.07077 |

Best varlen+TTT (**1.07183**) now improves over the varlen sliding baseline by **−0.00242 BPB** (target was −0.003), beating every other varlen entry on the leaderboard. It does **not yet** beat the non-varlen+TTT record (1.07077); the residual gap is in the **varlen quant→sw step**, not in TTT itself.

**Follow-ups (moved to `program.md` TTT sweep section):**
1. Adapter-placement re-ablation (`TTT_K_LORA / TTT_MLP_LORA / TTT_O_LORA`) at the new rank=48 default.
2. Fine LR / `TTT_GRAD_STEPS` re-scan around the new optimum.
3. Compose with SLOT (`SLOT_ENABLED=1`).
4. Investigate the varlen quant→sw delta directly — that is now the dominant gap, not TTT.

The historical investigation notes follow for reference.
