...
# fla and causal_conv1d use custom autograd.Functions; disable torch.compile
# for the full model so these kernels can run natively.
...
def torch_chunk_gated_delta_rule(query, key, value, g, beta, chunk_size=64, use_qk_l2norm_in_kernel=True):
    ...
    eye = torch.eye(chunk_size, dtype=query.dtype, device=query.device)
    for ci in range(n_chunks):
        ...
        v_c = value[:, :, s:e]
        b_c = beta[:, :, s:e]
        g_c = g[:, :, s:e].cumsum(dim=-1)
        v_beta = v_c * b_c.unsqueeze(-1)
        k_beta = k_c * b_c.unsqueeze(-1)
        # Per-chunk decay mask
        decay = ((g_c.unsqueeze(-1) - g_c.unsqueeze(-2)).tril().exp()).tril()
        # Intra-chunk correction
        A = -((k_beta @ k_c.transpose(-1, -2)) * decay).masked_fill(triu_mask, 0)
        ...
        A = A + eye
        v_corr = A @ v_beta
        k_cumdecay = A @ (k_beta * g_c.exp().unsqueeze(-1))
        # Inter-chunk
        attn_c = q_c @ k_c.transpose(-1, -2) * decay
        v_prime = k_cumdecay @ state
        v_new = v_corr - v_prime
        attn_inter = (q_c * g_c[..., None].exp()) @ state
        outputs.append(attn_inter + attn_c @ v_new)
        # Update recurrent state
        state = (
            state * g_c[:, :, -1, None, None].exp()
            + (k_c * (g_c[:, :, -1, None] - g_c).exp()[..., None]).transpose(-1, -2) @ v_new
        )
    out = torch.stack(outputs, dim=2).reshape(B, H, -1, Dv)[:, :, :T]
    return out.transpose(1, 2).contiguous().to(initial_dtype)


@torch.compiler.disable
def _fla_delta_rule_wrapper(query, key, value, g, beta):
    out, _ = fla_chunk_gated_delta_rule(query, key, value, g=g, beta=beta,
...
@torch.compiler.disable
def _causal_conv1d_wrapper(x, weight):
    return causal_conv1d_fn(x=x, weight=weight, bias=None, activation="silu")
...
class GatedDeltaNet(nn.Module):
    ...
    def forward(self, x):
        ...
        if _use_causal_conv1d and causal_conv1d_fn is not None:
            mixed_qkv = _causal_conv1d_wrapper(mixed_qkv, self.conv1d.weight.squeeze(1))
        else:
        ...
        if _use_fla and fla_chunk_gated_delta_rule is not None:
            out = _fla_delta_rule_wrapper(query, key, value, g, beta)
        else:
