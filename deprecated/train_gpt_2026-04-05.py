...
def attn_res(sources: list[tuple[Tensor, Tensor]], proj_weight: Tensor) -> Tensor:
    ...
    for src, normed_src in sources:
        score = torch.einsum('d,btd->bt', proj_weight, normed_src)
        logits.append(score)
        running_max = score if running_max is None else torch.maximum(running_max, score)
    exp_sum = torch.zeros_like(running_max)
    mixed = torch.zeros_like(sources[0][0])
    for score, (src, _) in zip(logits, sources):
        weight = torch.exp(score - running_max)
        exp_sum = exp_sum + weight
        mixed = mixed + weight.unsqueeze(-1) * src
    return mixed / exp_sum.unsqueeze(-1)
...
class Block(nn.Module):
    ...
    def forward(self, sources: list[tuple[Tensor, Tensor]]) -> list[tuple[Tensor, Tensor]]:
        attn_input = attn_res(sources, self.attn_res_proj.to(dtype=sources[0][0].dtype))
        attn_output = attn_input + self.attn(self.attn_norm(attn_input) * self.ln_scale_factor)
        sources = sources + [(attn_output, normalize_source(attn_output))]
        mlp_input = attn_res(sources, self.mlp_res_proj.to(dtype=sources[0][0].dtype))
        mlp_output = mlp_input + self.mlp(self.mlp_norm(mlp_input) * self.ln_scale_factor)
        return sources + [(mlp_output, normalize_source(mlp_output))]
...
class GPT(nn.Module):
    ...
    def forward_logits(self, input_ids: Tensor) -> Tensor:
        ...
        if self.embed_proj is not None:
            x = self.embed_proj(x)
        sources: list[tuple[Tensor, Tensor]] = [(x, normalize_source(x))]
        for block in self.blocks:
            sources = block(sources)
        x = self.final_norm(sources[-1][0])
