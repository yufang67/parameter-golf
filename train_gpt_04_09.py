...
class Hyperparameters:
    ...
    turboquant_quantize_embeddings = bool(int(os.environ.get("TURBOQUANT_QUANTIZE_EMBEDDINGS", "0")))
...
def _turboquant_quantizer_for_dim(dim, h, cache):
    key = (h.turboquant_mode, dim, h.turboquant_bits, h.turboquant_seed, h.turboquant_norm_correction)
    quantizer = cache.get(key)
    if quantizer is not None:
        return quantizer
    TurboQuant, TurboQuantMSE, _ = _load_turboquant_symbols()
    if h.turboquant_mode == "full":
        quantizer = TurboQuant(dim, bit_width=h.turboquant_bits, seed=h.turboquant_seed, norm_correction=h.turboquant_norm_correction)
    elif h.turboquant_mode == "mse":
        quantizer = TurboQuantMSE(dim, bit_width=h.turboquant_bits, seed=h.turboquant_seed, norm_correction=h.turboquant_norm_correction)
    else:
...
def turboquant_mixed_quantize(state_dict, h):
    ...
    for name, tensor in state_dict.items():
        ...
        if not _turboquant_should_quantize(name, t, h):
            ...
            continue
        quantizer = _turboquant_quantizer_for_dim(t.shape[1], h, quantizer_cache)
        rows = t.float().numpy()
        norms_dtype = np.float16
        base_meta = {
            "bit_width": h.turboquant_bits,
        ...
        if h.turboquant_mode == "full":
            ...
            result[name + ".mse_indices_packed"] = torch.from_numpy(_pack_lowbit_array(mse_indices, h.turboquant_bits - 1))
            ...
            result[name + ".indices_packed"] = torch.from_numpy(_pack_lowbit_array(indices, h.turboquant_bits))
    ...
    for name, orig in template_sd.items():
        ...
        if isinstance(info, dict):
            ...
            if kind in ("turboquant_mse", "turboquant_full"):
                ...
                quantizer = _turboquant_quantizer_for_dim(shape[1], type("Q", (), info | {
...
def serialize(h, base_model, code):
    ...
    if h.quantizer == "gptq":
        ...
        log(f"TurboQuant:{h.turboquant_mode} bit_width={h.turboquant_bits} quantizing eligible 2D tensors")
